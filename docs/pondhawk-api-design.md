...
`IRequestContext` (Pondhawk's `ICorrelation` equivalent), **lean to start**: `CorrelationId`, `Caller` (`ClaimsPrincipal?`), `CallerGatewayToken` (`string?`). No debug-probe/level fields in v1. Scoped, registered by `AddPondhawkApi()`, populated by the enrichment middleware. Used by the response filter (correlation id on ProblemDetails), exception handler, and diagnostics middleware. (`CallerGatewayToken` is captured by enrichment but currently unconsumed — it's the hook a future outbound propagation would use.) Correlation-id reuses Pondhawk.Logging's `CorrelationManager`/`Activity`.
...
- Shared identity primitives (used by the inbound handlers, and by any future outbound propagation): `ClaimSet`, `IdentityConstants`, `IGatewayTokenEncoder`/`GatewayTokenJwtEncoder` (HS256 via IdentityModel), `ClaimSetPrincipal` (claim↔principal mapping). These stay in `Pondhawk.Api` for now because their only current consumer is the inbound auth (also in `Pondhawk.Api`).
- **Outbound/client propagation: dropped from v1.** `GatewayTokenHttpHandler` (a `System.Net.Http` `DelegatingHandler`), `IAccessTokenSource`, and the two header/token builder middlewares were removed — they are an HTTP-*client* concern (and the builder middlewares are gateway/proxy-only), not a web-server/API concern, and there is no current service-to-service need. If/when Pondhawk services need to call other services carrying an identity, extract a light **`Pondhawk.Identity`** package (no ASP.NET) holding the primitives above + the client propagation, and have `Pondhawk.Api` reference it for the inbound handlers. Until then, the primitives rightly live with their sole consumer in `Pondhawk.Api`.
...
**Resolved:** one unified package · DI-activated endpoint modules · drop jose-jwt (Microsoft.IdentityModel, HS256) · drop Autofac (MS.DI) · support both header & token gateway modes · **outbound/client identity propagation dropped from v1** (not an API concern; no current service-to-service need — extract `Pondhawk.Identity` later if it arises) · gateway token is HS256 symmetric (confirmed).
